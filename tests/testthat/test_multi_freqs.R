...
test_that("multi_freqs - grouped ns and percentages", {
  ...
  expected_n <- responses2 %>% dplyr::filter(gender == 'other') %>%
...
test_that("multi_freqs - digits argument", {
  test_3 <- responses2 %>% multi_freqs(m_activity_1, digits = 3) %>%
    dplyr::filter(label == 'Baseball') %>%
    dplyr::pull(result)
  test_2 <- responses2 %>% multi_freqs(m_activity_1) %>%
    dplyr::filter(label == 'Baseball') %>%
    dplyr::pull(result)
  test_1 <- responses2 %>% multi_freqs(m_activity_1, digits = 1) %>%
    dplyr::filter(label == 'Baseball') %>%
    dplyr::pull(result)
